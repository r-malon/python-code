...
- RadioButtons for selecting background history (son of: noble, peasant, clergy etc.)
- Castle and Village upgrades
- Special places like monasteries, bandit camps, old ruins and caves
- Intelligence, Agility, Charisma, Strength (use GURPS or SPECIAL?)
- Jobs for villagers, like Blacksmith, Woodcutter or Farmer
- Taxes, Trade Taxes, Honour (slavery, culture), Renown, Laws, Political Power etc.
...
- Add family management (give daughter's hand in marriage etc.)
- and more...
...
class Character:
	...
	def status(self):
		return f'{self.name}\'s current health is {self.health}.'
	def __repr__(self):
		return 'Do something with the object!'

'''
class Player(Character): # instance or subclass
	def __init__(self):
	...
	def sleep(self):
		if self.stamina <= 5:
			return f'{self.name} is already fresh!'
		else:
			self.stamina += 1
...
class Troop(Character): # put it under Army?
	...
	def flee(self): # move it all to Army?
		self.chief = None
		self.payment = None
		return f'One {self.name} fled to the woods.'
	'''
...
class Castle: # shouldn't inherit? yeah
	def __init__(self, name, faction, lord):
		...
		# self.totalpop, self.civpop, self.milpop, etc
	...
	def recruit(self, troopnumber):
		...
		return f'You recruited {troopnumber} militiamen.'
	...
	def jobs(self, percentage_list):
		...
		for i, j in zip(list(jobs), percentage_list):
			jobs[i] = j
		return jobs
	def collect_tax(self, tax_level):
		self.lord.coins += self.population * tax_level
		self.population -= ceil(tax_level) # people leave when taxes are abusive
	...
	def feed(self, sacks): # troops also deserve food!
		...
		print(f'You need to feed {mouths} hungry jaws!') # move this to main.py?
		if self.warehouse['food'] < 2:
			dead = randint(starved // 2, starved - 2)
			self.lord.honour -= 5
			self.population -= dead
			return f'You have nothing to feed your people!!!\n{dead} peasants died or fled'
			...
			return f'After hearing about the plenty of grain in your lands, {newpeople} wandering peasants decide to live in {self.name} castle!'
		elif starved > 10:
			self.lord.honour -= 1
		self.warehouse['food'] -= sacks
		self.population -= starved
		return f'You have fed {sacks} people, but {starved} starved.'
	def harvest(self):
		self.warehouse['food'] += 2 * self.population * self.weather()[1]
		return self.weather()[0]
	def chop_wood(self): # put it on harvest?
